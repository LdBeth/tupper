  ...
  (cond
    ...
    ((consp expr)
     (let ((op (car expr)) (args (cdr expr)))
       (case op
         (+ (cond ((null args) (list (number->iv 0)))
                  ((null (cdr args)) (eval-expr (car args) x y))
                  (t (reduce (lambda (acc e)
                               (ivs-apply-binary #'iv-add acc (eval-expr e x y)))
                             (cdr args)
                             :initial-value (eval-expr (car args) x y)))))
         (- (cond ((null args) (error "(-) with no args"))
                  ((null (cdr args)) (ivs-apply-unary #'iv-neg (eval-expr (car args) x y)))
                  (t (reduce (lambda (acc e)
                               (ivs-apply-binary #'iv-sub acc (eval-expr e x y)))
                             (cdr args)
                             :initial-value (eval-expr (car args) x y)))))
         (* (cond ((null args) (list (number->iv 1)))
                  ((null (cdr args)) (eval-expr (car args) x y))
                  (t (reduce (lambda (acc e)
                               (ivs-apply-binary #'iv-mul acc (eval-expr e x y)))
                             (cdr args)
                             :initial-value (eval-expr (car args) x y)))))
...
(defun eval-expr-point (expr x y)
  "Evaluate EXPR at the *single point* (x, y) by passing degenerate ivals.
   Returns a single double-float or NIL if undefined/non-finite encountered."
    ...
    (when (and ivs (= (length ivs) 1))
      (let ((iv (car ivs)))
        (when (and (ival-totally-defined-p iv)
                   ...
                   ;; tight-ish (ok for sampling)
                   t)
          (* 0.5d0 (+ (ival-lo iv) (ival-hi iv))))))))

(defun ivt-proves-equation-p (formula x-lo x-hi y-lo y-hi)
  "Sample (L - R) at the four corners of the box; if signs differ
   over a continuous evaluation, return T."
